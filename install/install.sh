...
temp_source    = average # 曲线与紧急判据统一用哪个温度：
                         #   average = 全核平均（默认）· max = 最热核
                         # 实测温差 2.0~7.8°C（高负载曾达 11.7°C）
                         #   ⇒ average: 风扇更安静；但平均到 90°C 时最热核已约 92~102°C
                         #   ⇒ max:     风扇更早升速，对局部热点更保守
                         # ⚠️ 无论哪个，macOS/SoC 自身的硬件过热保护都仍然生效
