...
| 睡眠期间手动模式是否保持 | ✅ **已验证（是，且这是个坑）**：2026-09-14 合盖睡眠 44 分钟（Clamshell Sleep · 电池），用户态全程冻结（TempMonitor 应有 ~264 条日志，实际 **0 条**）⇒ 守护一次没写；而 `F*md=1` 锁存不回退 ⇒ 风扇按睡前的 3822 RPM 一直转。**已修**：守护注册 `IORegisterForSystemPower`，`kIOMessageSystemWillSleep` 时写 `F*md=0` 交还固件，唤醒后重新接管并清空 EMA/限幅历史 |
| 唤醒后固件是否重置 `F*md` | 无需依赖：主循环每拍读 `md`，≠1 就写回 1（唤醒路径另有显式 `takeover_fans()`）|
